...
Default certification is family-aware with retention probes (P2.3R-bis), with the recent-split rule in shadow. `--certification family_scoped` selects family-scoped activation (below). The compiler's cadence and thresholds are the P2.x values; nothing is lowered for integration.
...
Argument synthesis by the reflex, generative content, re-certification of an active family on its probes alone, a per-family minimum evidence count, trust extension into capable-but-unevidenced regions, persistent registry integration, and any change to historical benchmarks.
